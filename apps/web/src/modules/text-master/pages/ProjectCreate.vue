...
import { listTextTypes } from '../../../api/projects';
import type { TextProjectType, TextType } from '@text-master/shared';
...
const apiTextTypes = ref<TextType[]>([]);

const stepLabels = ['选择项目类型', '基础信息', '模板与设定', '完成创建'] as const;

const defaultProjectTypes: ProjectTypeOption[] = [
  {
    id: 'short-drama',
    value: 'short_drama',
    icon: '剧',
  ...
  },
  {
    ...
    icon: 'BP',
    title: '商业 BP',
    description: '项目概述、市场分析、融资方案',
    frequency: '5%',
    detail: '适合商业计划书、融资材料和路演文本，重点组织项目价值、市场空间、商业模式和融资计划。',
    accent: 'green',
  },
  {
    id: 'investment-copy',
    value: 'investment_copy',
    icon: '招',
    title: '招商文案',
    description: '加盟政策、产品卖点、招商页',
    frequency: '5%',
    detail: '适合招商加盟、渠道拓展和落地页文案，重点表达品牌定位、收益模型和合作政策。',
    accent: 'amber',
  },
  {
    id: 'academic-report',
...
const projectTypeCatalog = Object.fromEntries(
  defaultProjectTypes.map((item) => [item.id, item] as const),
) as Record<string, ProjectTypeOption>;

const apiTextTypeToCardId: Record<string, string> = {
  short_drama: 'short-drama',
  novel: 'novel',
  business_copy: 'business-copy',
  document: 'document',
  xiaohongshu: 'xiaohongshu',
  business_bp: 'business-bp',
...
const projectTypes = computed<ProjectTypeOption[]>(() => {
  const apiIds = apiTextTypes.value.length > 0
    ? apiTextTypes.value.map((item) => apiTextTypeToCardId[item.id] ?? item.id)
    : [];
  const ids = [...new Set([...apiIds, ...defaultProjectTypes.map((item) => item.id)])];

  return ids
    .map((id) => projectTypeCatalog[id])
    .filter((item): item is ProjectTypeOption => Boolean(item));
});
...
const selectedType = computed(
  () => projectTypes.value.find((item) => item.id === selectedTypeId.value) ?? projectTypes.value[0],
);
...
onMounted(async () => {
  try {
    apiTextTypes.value = await listTextTypes();
  } catch {
    apiTextTypes.value = [];
  }

  if (!projectTypes.value.some((item) => item.id === selectedTypeId.value) && projectTypes.value[0]) {
    selectedTypeId.value = projectTypes.value[0].id;
    selectedTemplateId.value = templateMap[projectTypes.value[0].value][0]?.id ?? 'skip';
  }
});
...
function selectQuickTemplate(template: TemplateShortcut): void {
  ...
  if (template.projectTypeId) {
    const projectType = projectTypes.value.find((item) => item.id === template.projectTypeId);
    if (projectType) {
  ...
  <main class="tm-create-page" data-testid="text-project-create">
      ...
      <section class="tm-create-main">
        <article class="tm-create-stage" data-testid="create-step-type">
          ...
          <section v-if="currentStep === 1" class="tm-step-panel">
            ...
            <div class="tm-type-grid" data-testid="create-project-type-grid">
              <button
                v-for="item in projectTypes"
                :key="item.id"
                type="button"
                :data-testid="`create-project-type-card-${item.id}`"
    ...
    <footer class="tm-create-footer">
      ...
      <div>
        ...
        <RouterLink class="tm-button" :to="textMasterRoutePaths.home" data-testid="create-cancel-button">取消</RouterLink>
        <button
          v-if="currentStep < 4"
...
.tm-template-strip button strong {
  font-size: 13px;
  line-height: 1.06;
}
...
:deep(.tm-top-nav__brand) {
  gap: 8px;
}

:deep(.tm-top-nav__mark) {
  width: 32px;
  height: 32px;
  border-radius: 12px;
  font-size: 12px;
...
:deep(.tm-top-nav__brand-copy strong) {
  font-size: 14px;
}

:deep(.tm-top-nav__brand-copy span) {
  font-size: 10px;
}

:deep(.tm-top-nav__links) {
  gap: 4px;
}

:deep(.tm-top-nav__links a),
:deep(.tm-top-nav__pill),
:deep(.tm-top-nav__profile),
:deep(.tm-top-nav__notify) {
  min-height: 28px;
  padding: 0 8px;
  font-size: 11px;
}

:deep(.tm-top-nav__right) {
  gap: 4px;
}
...
:deep(.tm-top-nav__avatar) {
  width: 20px;
  height: 20px;
  font-size: 10px;
}

@media (max-height: 920px) and (min-width: 1021px) {
  .tm-create-page {
    gap: 4px;
...
@media (max-height: 860px) and (min-width: 1021px) {
  ...
  .tm-create-layout {
    gap: 3px;
    grid-template-columns: 162px minmax(0, 1fr) 226px;
  }
