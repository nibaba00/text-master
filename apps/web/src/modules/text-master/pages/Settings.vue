...
const settingsFeedback = ref('设置页已加载，Provider 配置来自后端运行时。');
const loadError = ref('');
const testingConnection = ref(false);
const testResult = ref<{ ok: boolean; message: string } | null>(null);

// --- Provider 配置 ---
const providers = ref<BackendProviderSummary[]>([]);
const activeProviderId = ref<BackendProviderId>('mock-provider');
...
async function loadProviderSettings(): Promise<void> {
  const config = await getBackendProviderConfig();
  providers.value = config.providers;
  activeProviderId.value = config.activeProviderId;

  const deepseek = config.providers.find((provider) => provider.id === 'deepseek');
  deepseekSettings.apiKey = '';
  deepseekSettings.baseUrl = deepseek?.baseUrl ?? 'https://api.deepseek.com';
  deepseekSettings.model = deepseek?.model ?? 'deepseek-chat';
}

async function onProviderChange(providerId: BackendProviderId): Promise<void> {
  activeProviderId.value = providerId;
  const config = await updateBackendProviderConfig({ activeProviderId: providerId });
  providers.value = config.providers;
  activeProviderId.value = config.activeProviderId;
...
async function saveProviderSettings(): Promise<void> {
  const updated = await updateBackendProviderConfig({
    activeProviderId: activeProviderId.value,
    deepseek: {
      ...(deepseekSettings.apiKey.trim() ? { apiKey: deepseekSettings.apiKey } : {}),
      baseUrl: deepseekSettings.baseUrl,
      model: deepseekSettings.model,
    },
  });
  providers.value = updated.providers;
  activeProviderId.value = updated.activeProviderId;

  const deepseek = updated.providers.find((provider) => provider.id === 'deepseek');
  const isDeepseek = updated.activeProviderId === 'deepseek';
  const hasKey = Boolean(deepseek?.configured);
  settingsFeedback.value = isDeepseek && hasKey
    ? `DeepSeek 配置已保存。模型：${deepseek?.model ?? deepseekSettings.model}`
    : 'Provider 配置已保存。';
  deepseekSettings.apiKey = '';
  testResult.value = null;
}
...
async function onTestConnection(): Promise<void> {
  ...
  testResult.value = null;
  try {
    if (deepseekSettings.apiKey.trim() || activeProviderId.value === 'deepseek') {
      await saveProviderSettings();
    }
    testResult.value = await testBackendProvider(activeProviderId.value);
  } catch (error) {
    testResult.value = {
      ok: false,
      message: error instanceof Error ? error.message : '连接测试异常。',
    };
  } finally {
  ...
  <main class="tm-settings-page" data-testid="text-master-settings">
    ...
    <section class="tm-settings-grid">
      ...
      <article class="tm-settings-card tm-ai-card">
        ...
        <label>
          当前 Provider
          <select :value="activeProviderId" @change="onProviderChange(($event.target as HTMLSelectElement).value as BackendProviderId)">
            <option v-for="pv in providers" :key="pv.id" :value="pv.id">
              {{ pv.name }} {{ pv.available ? '' : '(未配置)' }}
            </option>
          </select>
        </label>
        ...
        <p v-if="activeProviderId === 'mock-provider'" class="tm-provider-note">
